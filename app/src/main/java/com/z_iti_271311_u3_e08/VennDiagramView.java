...
public class VennDiagramView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Escalar proporcionalmente al tamaño mínimo del canvas
        float scaleFactor = Math.min(width, height) / 1080f; // Escala basada en referencia (1080px)
        float baseRadius = scaleFactor * 200; // Radio base escalado

        // Coordenadas de los círculos principales
        float centerX1 = width / 2f;          // Círculo superior
        float centerY1 = height / 2.5f;

        float centerX2 = width / 2.5f;          // Círculo inferior izquierdo
        float centerY2 = 2 * height / 3f;

        float centerX3 = 2 * width / 3.5f;      // Círculo inferior derecho
        float centerY3 = 2 * height / 3f;
        ...
        addConcentricCircles(centerX3, centerY3, baseRadius);
        ...
        // Calcular y validar puntos de intersección
        List<PointF> intersectionPoints = calculateAllIntersections();
        validateIntersectionPoints(intersectionPoints, centerX1, centerY1, baseRadius);

        // Configurar y dibujar bloques de intersección
        setupBlocks(intersectionPoints);
        drawBlocks(canvas, scaleFactor);
    }
